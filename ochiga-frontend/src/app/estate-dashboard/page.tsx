...
type ChatMessage = {
  ...
  role: "user" | "assistant";
  content: string;
  panel?: string | null; // when present, this message is a panel block
  panelTag?: string | null; // grouping tag for panel blocks
...
export default function EstateDashboard() {
  ...
  // Move an existing panel group (all messages with panelTag) to bottom and update time
  const movePanelGroupToBottom = (panelTag: string) => {
    setMessages((prev) => {
      const group = prev.filter((m) => m.panelTag === panelTag);
      if (!group.length) return prev;

      // remove group and re-append with updated times
      const rest = prev.filter((m) => m.panelTag !== panelTag);
      const t = nowTime();
      const updatedGroup = group.map((m) => ({ ...m, time: t, id: createId() })); // new ids so key collisions avoided
      return [...rest, ...updatedGroup];
    });

    // scroll (if at bottom) or show scroll indicator
    setTimeout(() => {
  ...
  // Append a new panel group: user msg, assistant reply, panel block
  ...
  // Handle structured actions (from voice / footer)
  // Minimal mapping for estate-level targets; expands easily
  const handleAction = (actions: Array<{ type: string; action: string; target: string }>, userMessage?: string) => {
    actions.forEach((a) => {
      ...
      if (panel) {
        ...
        // plain text assistant reply (no panel)
        const t = nowTime();
        const userMsg: ChatMessage = {
          id: createId(),
          role: "user",
  ...
  // Core send handler (called by footer / dynamic suggestions / programmatically)
  function handleSend(text?: string) {
    ...
    // Simple heuristic to map plain commands to panel + reply text
    if (panel) {
      const reply =
        panel === "estate_devices"
    ...
    // Non-panel text -> simple chat exchange
    const t = nowTime();
    const userMsg: ChatMessage = {
      id: createId(),
      role: "user",
  ...
  return (
    <LayoutWrapper>
      {/* Top hamburger (push behavior handled via CSS in your layout/hamburger component) */}
      ...
      <main className="flex-1 flex flex-col justify-between relative overflow-hidden">
              ...
              return (
                ...
                >
                  <div className="flex flex-col max-w-[80%]">
                    {/* Chat bubble (user or assistant normal message) */}
                    {!isPanelBlock && (
                      <div
                    ...
                    {/* Panel block (assistant message that contains panel UI) */}
                    {isPanelBlock && (
        ...
        {/* Dynamic suggestion card just above footer */}
